...
{
    boolean leftup;  //used to check left paddle is moving up
    boolean leftdown;  //used to check left paddle is moving down
    boolean rightup;  //used to check right paddle is moving up
    boolean rightdown; //used to check right paddle is moving down

    int leftx, lefty, rightx, righty; //these variables are used to store x and y coordinates of paddle


    Timer time;  //used to create  a timer object
    int delay=100;  //used to store delay for timer

    boolean inGame;  //used to check if game is in progress
    

    boolean ballLeft;  //used to check if ball is moving left
    boolean ballright;  //used to check if ball is moving right
    boolean ballup;  //used to check if the ball is moving up
    boolean balldown;  //used to check if the ball is moving down
    
    int ballx, bally;  //these variables are used to x and y coordinates of ball

    int flag=0;  //used to check if the ball has hit the paddle
    int win=0;  //used to check if the game has won 
    ...
    public void move()
    {
        ...
        if(ballright&&ballup) //if the ball is moving up and right
        {
            if(bally==0)
        ...
        if(ballLeft&&ballup) //if the ball is moving up and left
        {
            if(bally==0)
            { //if the ball is at the top of the board make it move down
                ballup=false;
        ...
        if(ballLeft&&balldown) //if the ball is moving left and down
        {
            if(bally==600)
            { //if the ball is at the bottom of the board make it move up
                ballup=true;
            ...
            {//if the ball is at the border of the board  move it  diagonally
                ballx-=10;
                bally+=10;
            }
            return;
    ...
    {
        if(ballx<=0)
        {//if the ball is at the left border the right player wins
            inGame=false;
            win=1;
        }
        if(ballx>=1100)
        ...
        if(ballx==rightx)
        {//if ball is at right paddle make it move left
        ...
        if(ballx==leftx)
        { //if the ball is at left paddle make it move right
    ...
    //this method is used to paint the components of the board
